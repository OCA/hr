...
class HrPayslip(models.Model):
    ...
    def reset_amendments(self):
        amendments = self.env['hr.apyslip.amendment'].search(
            [
                ('slip_id', 'in', self.ids),
                ('state', '=', 'done')
            ]
    ...
    def get_inputs(self, contract_ids, date_from, date_to):
        ...
        amendments = amendment_pool.search(
            [
                ('state', '=', 'validate'),
                ('date', '>=', date_from),
                ('date', '<', date_to),
        ...
        for contract in contracts:
            for amendment in amendments:
                ...
                if code in input_map[contract.id]:
                    res[input_map[contract.id][code]]['amount'] = (
                        amendment.category_id.type == 'add'
                        and amendment.amount
                        or -amendment.amount
                    )
        return res
