...
class WellnessCheck(models.Model):
    ...
    q1_answer = fields.Text("Answer 1")
    q2_answer = fields.Text("Answer 2")
    q3_answer = fields.Text("Answer 3")
    ...
    def _compute_sentiment(self):
        ...
        for record in self:
            ...
            # Prioritizes high/low mood scores but allows keywords to shift.
    ...
    def get_wellness_stats(self):
        ...
        - 3-Day Trend: Qualitative assessment comparing recent avg vs previous.
        ...
        avg_mood_today = (
            sum(today_records.mapped("mood_score")) / participation_today
            if participation_today > 0
            else 0
        )
        ...
        last_3_days = self.search(
            [("date", ">", three_days_ago), ("date", "<=", today)]
        )
        prev_3_days = self.search(
            [("date", ">", six_days_ago), ("date", "<=", three_days_ago)]
        )

        last_avg = (
            sum(last_3_days.mapped("mood_score")) / len(last_3_days)
            if last_3_days
            else 0
        )
        prev_avg = (
            sum(prev_3_days.mapped("mood_score")) / len(prev_3_days)
            if prev_3_days
            else 0
        )
        ...
        return {
            ...
            "sign": sign,
            "trend_label": (
                f"{'+' if last_avg >= prev_avg else ''}"
                f"{round(last_avg - prev_avg, 1)} pts"
            ),
        }
