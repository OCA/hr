...
class ResUsers(models.Model):
    ...
    last_hr_wellness_check_date = fields.Date(
        ...
        help=(
            "The most recent date the user completed a sentiment check. "
            "Used to throttle daily popups."
        ),
    )
    ...
    def check_wellness_prompt(self):
        ...
        if (
            not self.env.user.last_hr_wellness_check_date
            or self.env.user.last_hr_wellness_check_date < today
        ):
            return True
