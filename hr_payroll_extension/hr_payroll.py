...
class hr_payslip(osv.osv):
    ...
    def leaves_list_get_hours(self, cr, uid, employee_id, contract_id, sched_tpl_id, d, 
        ...
        for l in leaves_list:
            ...
            if utcdt_dayS <= utcEnd and utcdt_dayE >= utcBegin:
                ...
                if utcBegin.date() < utcdt_dayS.date() and utcEnd.date() > utcdt_dayS.date():
                    ...
                    shift_times = self.pool.get('hr.schedule.detail').scheduled_begin_end_times(cr, uid,
                                                                                                employee_id,
                                                                                                contract_id,
                                                                                                dtS,
                                                                                                context=context)
                    if len(shift_times) > 0:
                        ...
                        hours = sched_tpl_obj.get_hours_by_weekday(cr, uid, sched_tpl_id, d.weekday(),
    ...
    def get_worked_day_lines(self, cr, uid, contract_ids, date_from, date_to, context=None):
        ...
        try:
            ...
            self._mtm_public_holidays_list = self.holidays_list_init(cr, uid, day_from, day_to,
                                                                     context=context)
            public_holidays_list = self._mtm_public_holidays_list
        ...
        for contract in self.pool.get('hr.contract').browse(cr, uid, contract_ids, context=context):
            ...
            working_hours_dict = self.attendance_dict_init(cr, uid, contract, day_from, day_to,
                                                           context=None)
            ...
            ot_data = None
            data2 = None
            ot_data = get_ot_policies(contract.policy_group_id, day_from, ot_data)
            ...
            absence_data = None
            data2 = None
            absence_data = get_absence_policies(contract.policy_group_id, day_from, absence_data)
            data2 = get_absence_policies(contract.policy_group_id, day_to, data2)
            ...
            for day in range(0, nb_of_days):
                ...
                #
                actual_rest_days = sched_obj.get_rest_days(cr, uid, contract.employee_id.id,
    ...
    def get_utilities_dict(self, cr, uid, contract, payslip, context=None):
        ...
        res.update({'PPF': {'amount': self._partial_period_factor(payslip, contract)}})
        
        # Calculate net amount of previous payslip
        imd_obj = self.pool.get('ir.model.data')
        ps_obj = self.pool.get('hr.payslip')
        ps_ids = ps_obj.search(cr, uid, [('employee_id', '=', contract.employee_id.id)],
    ...
    def get_payslip_lines(self, cr, uid, contract_ids, payslip_id, context):
        ...
        for contract in self.pool.get('hr.contract').browse(cr, uid, contract_ids, context=context):
            temp_dict = {}
            utils_dict = self.get_utilities_dict(cr, uid, contract, payslip, context=context)
            ...
            localdict.update({'employee': employee, 'contract': contract, 'utils': utils_obj})
