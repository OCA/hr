...
class TestHrPayslip(common.TransactionCase):
    def setUp(self):
        ...
        for payslip in [
            ...
            (1, '2014-01-01', '2014-01-31', False, self.employee.id, self.contract.id),
            (2, '2014-02-01', '2014-02-28', True, self.employee.id, self.contract.id),
            (3, '2014-03-01', '2014-03-31', False, self.employee.id, self.contract.id),

            # This payslip is the current one
            (4, '2014-04-01', '2014-04-30', False, self.employee.id, self.contract.id),

            # These payslips should not be summed over
            # One in 2013 and the other for another employee and the third for a draft payslip
            (5, '2013-12-01', '2013-12-31', False, self.employee.id, self.contract.id),
            (6, '2014-01-01', '2014-01-31', False, self.employee_2.id, self.contract_2.id),
            (7, '2014-01-01', '2014-01-31', False, self.employee.id, self.contract.id),
        ]:
    ...
    def test_payslip_ytd_amount(self):
        ...
        for line in payslip.line_ids:
            if line.code == 'TEST_1':
                self.assertEqual(line.amount_ytd, 130)
            elif line.code == 'TEST_2':
                self.assertEqual(line.amount_ytd, 140)
