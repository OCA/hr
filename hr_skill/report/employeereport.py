...
class employeereport(report_sxw.rml_parse):
        ...
        def _getDuration(self, eid):
                    res1 = self.pool.get('hr.employee').read(
                        self.cr, self.uid, eid,)
                    sdate = datetime.datetime.strptime(
                        res1['started'], '%Y-%m-%d')
                    if res1['leavedate']:
                        edate = datetime.datetime.strptime(
                            res1['leavedate'], '%Y-%m-%d')
                    else:
...
report_sxw.report_sxw('report.datereport.print', 'hr.employee',
                      'addons/hr_skill/report/datereport.rml', parser=employeereport, header=True)
