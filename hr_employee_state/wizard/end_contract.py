...
class employee_set_inactive(osv.TransientModel):
    ...
    def set_employee_inactive(self, cr, uid, ids, context=None):
        ...
        data = self.read(cr, uid, ids[0], ['employee_id', 'contract_id', 'date', 'reason_id', 'notes'],
                         context=context)
        vals = {
            'name': data['date'],
        ...
        contract = contract_obj.browse(cr, uid, data['contract_id'][0], context=context)
        contract_obj.setup_pending_done(cr, uid, contract, vals, context=context)
