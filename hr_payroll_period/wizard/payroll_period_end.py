...
class payroll_period_end_1(osv.osv_memory):
    ...
    def _get_change(self, cr, uid, context=None):
        ...
        if period_id:
            ...
            if data['register_id']:
                data = self.pool.get('hr.payroll.register').read(cr, uid, data['register_id'][0],
                                                                 ['run_ids'], context=context)
                if data['run_ids']:
                    ps_runs = self.pool.get('hr.payslip.run').browse(cr, uid, data['run_ids'],
                                                                     context=context)
                    for psr in ps_runs:
    ...
    def _get_public_holidays(self, cr, uid, context=None):
        ...
        start = datetime.strptime(data['date_start'], OEDATETIME_FORMAT).date().strftime(OEDATE_FORMAT)
        end = datetime.strptime(data['date_end'], OEDATETIME_FORMAT).date().strftime(OEDATE_FORMAT)
        ...
        for dept in dept_obj.browse(cr, uid, dept_ids, context=context):
            ...
            for data in c_data:
                if contracts_dict.get(data['employee_id'][0], False):
                    ...
                    contracts_dict.update({data['employee_id'][0]: [data['id']]})
            ...
            # Alphabetize
            ee_ids = ee_obj.search(cr, uid, [('id', 'in', ee_ids)], context=context)
