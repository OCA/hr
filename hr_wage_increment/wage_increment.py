...
class wage_increment(osv.osv):
    ...
    def _check_state(self, cr, uid, wage_incr, context=None):
        ...
        if len(wage_incr_ids) > 0:
            data = self.pool.get('hr.contract').read(cr, uid, wage_incr.contract_id.id, ['name'], context=context)
            raise osv.except_osv(_('Warning'), _('There is already another wage adustment in progress for this contract: %s.') % (data['name']))
        
        contract_obj = self.pool.get('hr.contract')
        data = contract_obj.read(cr, uid, wage_incr.contract_id.id, ['state', 'date_end'], context=context)
        
        if data['state'] in ['draft', 'done']:
            data = self.pool.get('hr.contract').read(cr, uid, wage_incr.contract_id.id, ['name'], context=context)
            raise osv.except_osv(_('Warning!'), _('The current state of the contract does not permit a wage change: %s')  % (data['name']))
        ...
        if data.get('date_end', False) and data['date_end'] != '':
            ...
            if dEffective >= dContractEnd:
                data = self.pool.get('hr.contract').read(cr, uid, wage_incr.contract_id.id, ['name'], context=context)
                raise osv.except_osv(_('Warning!'), _('The contract end date is on or before the effective date of the adjustment: %s') %(data['name']))
    ...
    def create(self, cr, uid, vals, context=None):
        ...
        data = self.pool.get('hr.contract').read(cr, uid, contract_id, ['name', 'date_start'],
                                                 context=context)

        # Check that the contract start date is before the effective date
        if vals['effective_date'] <= data['date_start']:
            raise osv.except_osv(_('Error'), _('The effective date of the adjustment must be after the contract start date. Contract: %s.') % (data['name']))
