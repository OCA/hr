...
class ResourceCalendarAttendance(models.Model):
    ...
    def get_action_date(self, in_out, employee_id):
        ...
        if self._is_now_allowed(now, employee_id):
            ...
            matching_date = self._get_matching_date(in_out, now, start_datetime, end_datetime)
            if matching_date:
                if in_out == 'in':
    ...
    def _is_now_allowed(self, now, employee_id):
        is_after_from = not self.date_from or fields.Date.from_string(self.date_from) <= now.date()
        is_before_to = not self.date_to or fields.Date.from_string(self.date_to) >= now.date()
        return is_after_from and is_before_to and not self._is_now_in_leave(now, employee_id)

    def _is_now_in_leave(self, now, employee_id):
        calendar_leaves_matches = (l.matches(now) for l in self.calendar_id.leave_ids if not l.resource_id)
        employee_leaves_matches = (l.matches(now) for l in employee_id.resource_id.leave_ids)
    ...
    def _get_matching_date(self, in_out, now, start, end):
        ...
        matching_date_gen = (datetime.combine(d.date(), now.time()) for d in dates_in_delta if
