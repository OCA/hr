    ...
    setup() {
        onWillStart(async () => {
            try {
                ...
                const shouldPrompt = await rpc(
                    "/web/dataset/call_kw/res.users/check_wellness_prompt",
                    {
                        model: "res.users",
                ...
                if (shouldPrompt) {
                    ...
                    setTimeout(() => {
                        action.doAction(
                            "hr_wellness_check.action_hr_wellness_check_wizard"
                        );
                    }, 500);
                }
            } catch (err) {
                ...
                console.warn(
                    "Wellness Check automated trigger suppressed an error:",
                    err
                );
            }
        });
...
registry
    ...
    .add("WellnessPulseTrigger", { Component: WellnessPulseTrigger });
