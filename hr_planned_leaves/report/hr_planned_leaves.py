...
class hrself_planned_leaves(report_rml):
    ...
    def create(self, cr, uid, ids, datas, context):
        ...
        for holidays_status in holidays_status_obj.browse(cr, uid, holidays_status_ids):
            rml += """
            <place x="%smm" y="-3.20cm"  width="100.0cm" height="5cm">
                <illustration x="%smm" y="-3.00cm" >
                    <fill color="%s"/>
                    <rect x="0.0cm" y="-0.38cm" width="%smm" height="0.4cm" fill="yes" stroke="no"/>
                </illustration>
            <para style="status_right">
                <font color="white">""" % (
                str(x),
                str(x),
                tools.ustr(holidays_status.color_name),
                len(holidays_status.name) * 2
            ) + tools.ustr(holidays_status.name) + """</font></para></place>"""
            x += len(holidays_status.name) * 2.1
        rml += """
                                <setFont name="Helvetica" size="9"/>
                                <drawString x="1.0cm" y="1cm">%s</drawString>
                                <drawString x="14.5cm" y="1cm">%s</drawString>
            ...
            <story><para style="blank_space">From : %s To : %s </para>""" % (
            time.strftime("%Y-%m-%d %H:%M:%S", time.localtime()),
            tools.ustr(user),
            datas['form']['date_from'],
            datas['form']['date_to'],
        )
        ...
        for year in years:
            for data in final_datas:
                ...
                rml += """<blockTable colWidths=" """ + \
                    tools.ustr(_cols_Widhts) + """ " style="month" repeatRows="1"><tr>"""
                            ...
                            for holiday in holidays_obj.browse(cr, uid, holidays_ids):
                                if holiday.date_from and holiday.date_to:
                                    ...
                                    if (final_date_from.day, final_date_to.day) not in from_to_lst:
                                        colors_dict.update({
                                            final_date_from.day: str(holiday.holiday_status_id.color_name),
                                            final_date_to.day: str(holiday.holiday_status_id.color_name),
                                        })
                                        from_to_lst.append(
