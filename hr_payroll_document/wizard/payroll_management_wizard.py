...
class PayrollManagamentWizard(models.TransientModel):
    ...
    payrolls = fields.Many2many('ir.attachment', 'payrol_rel', 'doc_id','attach_id3', string="Payrolls", copy=False)
    ...
    def send_payrolls(self):
        ...
        # Validate if each employee have identification ID
        for employee in self.env['hr.employee'].search([]):
            if not employee.identification_id:
                raise UserError(_('You must to filled identification ID field of all employees'))
        ...
        for page in reader.pages:
            for value in page.extract_text().split():
                if self.validate_id(value) and value != self.env.company.vat:
                    employee = self.env['hr.employee'].search([('identification_id', '=', value)])
                    if employee:
        ...
        if not_found:
            return {
                ...
                'params': {
                    ...
                    'message':  _("IDs whose employee has not been found: ") + ', '.join(list(not_found)),
                    'sticky': True,
                    'type':'warning',
                    'next': {
                        ...
                        'views': [(self.env.ref('hr.hr_employee_public_view_kanban').id,'list')]
                    },
                }
            }
        ...
        self.env['ir.attachment.payroll.custom'].create(
            {
                ...
                'employee': employee.name, 'month': self.month, 
                'identification_id': employee.identification_id
            })

        # Send mail
        mail_template = self.env.ref("hr_payroll_document.payroll_employee_email_template")
        context = {'month':  self.month }
        data_id = [(6, 0, [self.env['ir.attachment'].create(ir_values).id])]
        mail_template.attachment_ids = data_id
        mail_template.with_context(context).send_mail(employee.user_id.id, force_send=True)

    def validate_id(self, id):
        mod = get_cc_module(self.env.company.country_id.code, 'vat')
        return mod.is_valid(id)
