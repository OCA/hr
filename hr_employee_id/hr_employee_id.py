...
class hr_employee(orm.Model):
    ...
    def _rec_message_employee(self, cr, uid, ids, context=None):
        return _('The Employee Number must be unique accross the company(s).')

    def _rec_message_tin(self, cr, uid, ids, context=None):
        return _('There is already another employee with this TIN number.')

    _sql_constraints = [
        ('employeeno_uniq', 'unique(employee_no)', _rec_message_employee),
        ('tinno_uniq', 'unique(tin_no)', _rec_message_tin),
    ]
    ...
    def _generate_employeeno(self, cr, uid, arg):
        ...
        while tries < max_tries:
            ...
            eid = ''.join(
                rnd.choice(string.digits) for _ in xrange(self.IDLEN)
            )
            cr.execute(
                '''SELECT employee_no FROM hr_employee WHERE employee_no=%s''',
                tuple((eid,))
            )
            res = cr.fetchall()
            if len(res) == 0:
