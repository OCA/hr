...
class HrExpense(models.Model):
    ...
    allow_without_attachment = fields.Boolean(default=False, string='Allow lines without attachment')

    @api.multi
    def action_submit_expenses(self):
        for rec in self:
            if (rec.product_id.expense_receipt_required and \
                    not rec.attachment_number) and not rec.allow_without_attachment:
        ...
        for rec in self:
            for expense in rec.expense_line_ids:
                if (expense.product_id.expense_receipt_required and \
                        not expense.attachment_number) and not rec.allow_without_attachment:
