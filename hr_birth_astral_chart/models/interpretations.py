...
MOON_INTERPRETATIONS = [
    ...
    # Cancer
    _lt("Deeply intuitive and empathetic; home and family are the emotional anchor."),
    # Leo
    _lt("Needs recognition and warmth; generous and loyal in emotional bonds."),
    # Virgo
    _lt("Analytical about feelings; finds comfort in being useful and well-organised."),
    # Libra
    _lt(
        "Needs harmony and partnership; dislikes conflict and seeks emotional balance."
    ),
    # Scorpio
    _lt("Intense and private emotional world; deeply loyal but prone to jealousy."),
    # Sagittarius
    _lt(
    ...
    # Capricorn
    _lt("Reserved with emotions; finds security through achievement and structure."),
    # Aquarius
    _lt("Detached but humanitarian; connects best through shared ideas and ideals."),
    # Pisces
    _lt(
        "Extremely sensitive and empathetic; absorbs surrounding emotions like a "
        "sponge."
    ),
]
...
def build_interpretation(env, chart_data):
    ...
    dist_rows = "".join(
        f"<tr>"
        f"<td style='padding:2px 8px;color:{_ELEMENT_COLORS[el]}'>"
        f"{env._(ELEMENT_LABELS[el])}</td>"
        f"<td style='padding:2px 8px'>"
        f"{'◉ ' * element_counts[el]}</td></tr>"
        for el in ("Fire", "Earth", "Air", "Water")
    )
    mod_rows = "".join(
        f"<tr><td style='padding:2px 8px'>{env._(MODALITY_LABELS[mod])}</td>"
        f"<td style='padding:2px 8px'>"
        f"{'◉ ' * modality_counts[mod]}</td></tr>"
        for mod in ("Cardinal", "Fixed", "Mutable")
    )
    pol_rows = "".join(
        f"<tr><td style='padding:2px 8px'>{env._(POLARITY_LABELS[pol])}</td>"
        f"<td style='padding:2px 8px'>"
...
def build_transit_interpretation(env, natal_data, transit_data, aspects, today):
    ...
    for asp in tight:
        ...
        rows_html += (
            ...
            f"{env._(ASPECT_LABELS[asp['aspect']])}</td>"
