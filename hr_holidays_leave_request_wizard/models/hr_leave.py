...
class HrLeave(models.Model):
    ...
    def _default_get_request_parameters(self, values):
        """Context is needed to prevent change in some use case
        when it's not necessary these change"""
        if not self.env.context.get("bypass_override_dates"):
            ...
            values['date_from'] = date_from.replace(tzinfo=None)
            values['date_to'] = date_to.replace(tzinfo=None)

        return super()._default_get_request_parameters(values)
