...
class policy_ot(osv.Model):
    ...
    def get_codes(self, cr, uid, idx, context=None):
        ...
        res = []
        [res.append((line.code, line.name, line.type, line.rate)) for line in self.browse(cr, uid, idx, context=context).line_ids]
        return res
    ...
    def restday_codes(self, cr, uid, idx, context=None):
        ...
        res = []
        [res.append((line.code, line.name)) for line in self.browse(cr, uid, idx, context=context).line_ids if line.type == 'weekly' and line.active_after_units == 'day']
        return res
    ...
    def weekly_codes(self, cr, uid, idx, context=None):
        ...
        res = []
        [res.append((line.code, line.name)) for line in self.browse(cr, uid, idx, context=context).line_ids if line.type == 'weekly' and line.active_after_units == 'min']
        return res
...
class policy_line_ot(osv.Model):
    ...
    _columns = {
        ...
        'type': fields.selection([('daily', 'Daily'),
                                 ...
                                 'Type', required=True),
        'active_after': fields.integer('Active After'),
        'active_after_units': fields.selection([('min', 'Minutes'), ('day', 'Days')],
                                               'Active After Units'),
