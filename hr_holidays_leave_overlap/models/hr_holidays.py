    ...
    def create(self, values):
        ...
        for leave in left_overlapping:
            base_date[0] = fields.Datetime.to_string(
                fields.Datetime.from_string(leave.date_to) + timedelta(seconds=1)
            )
        ...
        for leave in included:
            ...
            start_date = stepping_date and stepping_date[-1] or base_date[0]
            end_date = fields.Datetime.to_string(
                fields.Datetime.from_string(leave.date_from) - timedelta(seconds=1)
            )
            split_date.append((start_date, end_date))
            stepping_date.append(
