...
class TestLeaveOverlap(common.TransactionCase):
    def setUp(self):
        ...
        # Create an allocation to make leave requests
        self.allocation_id = self.leave_allocation_model.create(
            {
                "name": "Test Allocation",
                "holiday_status_id": self.test_leave_type_id.id,
                "date_from": Datetime.to_string(self.today - timedelta(days=22)),
                "date_to": Datetime.to_string(self.today + timedelta(days=22)),
        ...
        self.leave_request1 = self.leave_request_model.create(
            {
                ...
                "holiday_allocation_id": self.allocation_id.id,
                "employee_ids": self.test_employee_id,
                "state": "draft",
                "category_id": False,
            }
        )
    ...
    def test_01_left(self):
        ...
        res = self.leave_request_model.search(
            [
                ("employee_id", "=", self.test_employee_id.id),
                ("date_from", "=", Datetime.to_string(self.today - timedelta(days=2))),
                ("date_to", "=", Datetime.to_string(self.today - timedelta(seconds=1))),
            ]
        )
        ...
        self.leave_request_model.create(
            {
                ...
                "holiday_type": "employee",
                "employee_id": self.test_employee_id.id,
                "date_from": Datetime.to_string(self.today + timedelta(days=10)),
                "date_to": Datetime.to_string(self.today + timedelta(days=12)),
                "holiday_allocation_id": self.allocation_id.id,
                "employee_ids": self.test_employee_id,
                "state": "draft",
                "category_id": False,
        ...
        res = self.leave_request_model.search(
            [
                ...
                (
                    "date_from",
                    "=",
                    Datetime.to_string(self.today + timedelta(days=11, seconds=1)),
                ),
                ("date_to", "=", Datetime.to_string(self.today + timedelta(days=12))),
            ]
        )
        ...
        self.leave_request_model.create(
            {
                ...
                "holiday_type": "employee",
                "employee_id": self.test_employee_id.id,
                "date_from": Datetime.to_string(self.today + timedelta(days=3)),
                "date_to": Datetime.to_string(self.today + timedelta(days=8)),
                "holiday_allocation_id": self.allocation_id.id,
                "employee_ids": self.test_employee_id,
                "state": "draft",
                "category_id": False,
        ...
        res = self.leave_request_model.search(
            [
                ...
                (
                    "date_from",
                    "=",
                    Datetime.to_string(self.today + timedelta(days=7, seconds=1)),
                ),
                ("date_to", "=", Datetime.to_string(self.today + timedelta(days=8))),
            ]
        )
            ...
            self.leave_request_model.create(
                {
                    ...
                    "holiday_type": "employee",
                    "employee_id": self.test_employee_id.id,
                    "date_from": Datetime.to_string(self.today + timedelta(days=5)),
                    "date_to": Datetime.to_string(self.today + timedelta(days=6)),
                    "holiday_allocation_id": self.allocation_id.id,
                    "employee_ids": self.test_employee_id,
                    "state": "draft",
                    "category_id": False,
        ...
        self.leave_request_model.create(
            {
                ...
                "holiday_type": "employee",
                "employee_id": self.test_employee_id.id,
                "date_from": Datetime.to_string(self.today - timedelta(days=1)),
                "date_to": Datetime.to_string(self.today + timedelta(days=13)),
                "holiday_allocation_id": self.allocation_id.id,
                "employee_ids": self.test_employee_id,
                "state": "draft",
                "category_id": False,
        ...
        res = self.leave_request_model.search(
            [
                ...
                (
                    "date_from",
                    "=",
                    Datetime.to_string(self.today + timedelta(days=2, seconds=1)),
                ),
                (
                    "date_to",
                    "=",
