...
class HrEmployee(models.Model):
    ...
    def work_scheduled_on_day(self, date_dt, public_holiday=True,
        ...
        if public_holiday and self.env['hr.holidays.public'].is_public_holiday(
            ...
            days = \
                self.get_work_days_count(datetime.combine(date_dt, time.min),
                                         datetime.combine(date_dt, time.max))
            if not days:
                return False
        elif schedule and (not self.contract_id or (self.contract_id and
                            not self.contract_id.resource_calendar_id)):
