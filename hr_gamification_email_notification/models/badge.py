...
class BadgeUser(models.Model):
    ...
    def _send_badge_awards_emails(self):
        ...
        template = self.env.ref(
            "osi_pm_hr_gamification.email_template_badge_weekly_awards"
        )
        date_before_aweek = fields.Date.today() - timedelta(days=6)
        channel_general.with_context(
            ...
            template.id, model=self._name, res_id=self.id, composition_mode="mass_mail"
        )
